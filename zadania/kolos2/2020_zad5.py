...
def trojki(T):
    licz=0
    def rek(T,w=[],pos=0,przerwa=False):
        if len(w)==3:
            if nwd(nwd(w[0],w[1]),w[2])==1:
                nonlocal licz
                licz+=1
                print(w)
                return 
        ...
        if przerwa==False:
            return rek(T,w,pos+1,True) or rek(T,w+[T[pos]],pos+1,False)
        return rek(T,w+[T[pos]],pos+1,False)
    rek(T)

    return licz

print(trojki([2,4,3,6,5]))
